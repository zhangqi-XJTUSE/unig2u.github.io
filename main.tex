...
\begin{enumerate}
    ...
    \item \textbf{Deep Mechanistic Insights:} Our analyses demystify when generation helps or harms understanding. We expose the inherent architectural trade-offs between generative and discriminative objectives, the vulnerabilities of the stepwise visual reasoning paradigm, and the critical role of intermediate alignment fidelity.
\end{enumerate}
...
Define the best stepwise protocol for $M$ as
\begin{equation}
\begin{aligned}
\mathrm{Perf}_{\mathrm{stepwise}}(M;\mathcal{D})
\triangleq
\max\Big\{
...
\paragraph{Unified prompting (Direct vs. Stepwise).}
Following the dual-protocol design in RealUnify, we evaluate all models under two prompt formats: \textbf{Direct} and \textbf{Stepwise}.
...
In \textbf{Stepwise} prompting, we explicitly decompose the instance into staged sub-prompts to diagnose the contribution of generation.
...
For most stepwise instances, we adopt a single-round interaction (generate once, then answer).
For inherently multi-step problems (e.g., jigsaw, maze), we use an iterative stepwise variant: if an instance requires $K$ steps, the model alternates between producing the step-$k$ intermediate visual artifact and the corresponding step-$k$ decision/answer, until reaching step $K$.
...
Most tasks in the stepwise setting allow at most \textbf{one} intermediate image per instance. For inherently multi-step tasks, we allow up to $K$ intermediate images (one per step).
...
For each model, we report results under the \textbf{Direct} and \textbf{Stepwise} prompting protocols (see Fig.~\ref{fig:stepwise-protocol}). \textbf{Direct} answers each instance with a single end-to-end prompt. \textbf{Stepwise} decomposes the process into stages (e.g., generating an intermediate image and then answering with it as additional context) to isolate the effect of intermediate generation. However, some models are unable to support the stepwise mode due to architectural or interface constraints. In such cases, we apply model-specific handling based on the underlying limitation; these cases and the corresponding treatments are described in detail in the model section. This paired design ensures that the comparison focuses on \emph{the effect of generation itself}, rather than differences in prompting, environment, compute, or tool availability.
...
\mathrm{Stepwise\ G2U\ Gain} \;&=\; \mathrm{Acc}^{\text{Stepwise}}_{\text{Model}} \; -\; \mathrm{Acc}_{\text{Base}}. \label{eq:stepwise_g2u_gain}
\end{align}
This definition isolates the net benefit introduced by the unified model beyond its underlying base model, separately under the \textbf{Direct} and \textbf{Stepwise} protocols. We compute G2U Gain both overall and per category/subtask to quantify when generation helps, is neutral, or harms understanding.

\paragraph{Stepwise intermediate effectiveness: RA and AL.}
To assess the usefulness of intermediate images in the stepwise protocol, we further introduce two alignment metrics evaluated by GPT-4o with customized prompts for each task family.
...
\begin{figure}[t]
    ...
    \caption{Rubric weights for stepwise intermediate evaluation. RA measures adherence and usefulness of the generated intermediate image w.r.t.\ the \texttt{generation\_prompt}, while AL measures consistency between the final answer, the generated image, and the original question.}
...
We benchmark representative closed-source unified models that natively integrate multimodal understanding and image generation within a single model interface (i.e., the model can directly follow the \textbf{Direct} and \textbf{Stepwise} protocols without external tool wiring). These models serve as strong ``all-in-one'' baselines for understanding-through-generation.
...
When a unified model does not have an official corresponding VLM base model (or its base model is not a vision-language model), we select the closest available VLM baseline in terms of training data and architecture, and use it as the base model for comparison.

\paragraph{Models evaluated in this work.}
We evaluate a diverse set of unified models, including: Emu3, X-Omni, UniWorld-V1, Mio, Bagel, OmniGen2, Lumina-DiMOO, Ovis-U1, OneCAT-3B, JavisGPT, UAE, TokenFlow-XL, UniPic2, Janus-Pro, Show-o2, ILLUME+, STAR-7B, MMaDA, FUDOK, AIA, Uni-Video, and MammothModa2. We also evaluate agentic unified systems such as \textbf{GPT-4o + GPT-image} ,\textbf{Nano banana-pro + gemini} and \textbf{Qwen + Qwen-Image-Edit}, .
...
All systems are evaluated under the same dataset, prompt templates, and budget constraints described in \cref{sec:eval-protocol}. When a model does not support a required capability (e.g., stepwise intermediate generation), we apply the model-specific handling described in the corresponding model entry and report results transparently.
...
\paragraph{RQ2: Stepwise vs.\ direct generation.}
For G2U tasks, do unified models benefit more from \emph{explicit} stepwise generation of intermediate auxiliary images, or can \emph{direct} generation achieve comparable understanding performance?
\par\noindent\textbf{Experiment design:} We run each unified model in both protocols (\textbf{Direct} vs.\ \textbf{Stepwise}; \cref{sec:eval-protocol}) under matched budgets, compare final-task performance, and quantify the delta between the two modes to isolate the contribution of explicit intermediate visualizations.
...
\par\noindent\textbf{Experiment design:} We evaluate stepwise intermediate outputs with the two rubric-based metrics defined in Sec.~\ref{sec:metrics}: \textbf{RA}  and \textbf{AL} . We then analyze how RA/AL relate to downstream task performance and to the stepwise-vs-direct gains.
...
OmniGen2(stepwise) & 41.25 & 13.00 & \textbf{41.50} & 21.91 & 79.00 & \textbf{26.40} & 34.52 & 31.87~\textcolor{blue}{(-$2.58$)} \\
UniWorld-V1(stepwise) & 24.00 & 19.00 & 34.50 & 24.72 & 80.00 & 24.80 & 39.35 & 32.96~\textcolor{blue}{(-$1.49$)} \\
OneCAT-3b(direct) & 39.50 & 17.00 & 32.00 & 22.05 & 76.68 & 25.20 & 34.55 & 31.15~\textcolor{blue}{(-$3.30$)} \\
OneCAT-3b(stepwise) & 33.50 & 8.50 & 29.00 & 20.69 & 74.00 & 23.80 & 33.10 & 28.80~\textcolor{blue}{(-$5.65$)} \\
JavisGPT(direct) & 32.00 & 21.00 & 26.50 & 23.72 & 78.60 & 21.60 & 35.52 & 30.72~\textcolor{blue}{(-$3.73$)} \\
uni-video(stepwise) & 34.00 & 21.50 & 29.00 & \textbf{29.33} & 61.00 & 20.00 & \textbf{42.76} & 34.25~\textcolor{blue}{(-$0.20$)} \\
UniPic2(stepwise) & 17.50 & 17.00 & 31.00 & 23.95 & 47.00 & 24.60 & 38.80 & 30.65~\textcolor{blue}{(-$3.80$)} \\
STAR-7B(stepwise) & 34.00 & 14.50 & 32.50 & 23.56 & 79.00 & 25.20 & 38.56 & 32.68~\textcolor{blue}{(-$1.77$)} \\
\midrule
\rowcolor{gray!12}
...
UAE(stepwise) & 23.00 & 7.50 & 32.50 & 20.33 & 71.00 & 22.80 & 34.68 & 28.61~\textcolor{blue}{(-$3.78$)} \\
Ovis-U1(direct) & \textbf{45.50} & \textbf{23.50} & 30.50 & \textbf{27.28} & \textbf{90.00} & 17.60 & 34.92 & 32.15~\textcolor{blue}{(-$0.24$)} \\
Ovis-U1(stepwise) & 21.00 & 16.50 & 17.00 & 18.00 & 32.00 & 20.40 & 30.56 & 24.19~\textcolor{blue}{(-$8.20$)} \\
\midrule
\rowcolor{gray!12}
...
mio(stepwise) & 15.00 & 0.00 & 28.00 & 8.94 & 13.00 & 21.60 & 24.94 & 19.00~\textcolor{blue}{(-$4.73$)} \\
\midrule
\rowcolor{gray!12}
...
bagel(stepwise) & 32.75 & \textbf{30.00} & 40.50 & \textbf{31.58} & 91.00 & \textbf{29.00} & 37.29 & \textbf{36.10}~\textcolor{red}{(+$2.75$)} \\
...
show-o2(stepwise) & 33.00 & 9.00 & 40.00 & 26.39 & 35.00 & 20.40 & 28.11 & 26.59~\textcolor{blue}{(-$6.76$)} \\
ILLUME+(direct) & 24.50 & 7.50 & 35.00 & 21.06 & 80.00 & 23.20 & 35.08 & 29.54~\textcolor{blue}{(-$3.81$)} \\
ILLUME+(stepwise) & 20.50 & 2.00 & 35.50 & 19.53 & 58.00 & 21.00 & 34.05 & 27.13~\textcolor{blue}{(-$6.22$)} \\
\midrule
\rowcolor{orange!8}
...
For certain models, the absence of a public editing interface or the lack of edit-related training data makes edit-style stepwise generation effectively out-of-distribution (OOD).
Including their Stepwise results in the main comparison table would therefore introduce unfair bias, as these models are not architecturally or training-wise designed to perform edit-based intermediate reasoning.
To ensure fairness in cross-model comparison, we exclude the Stepwise results of such models from the main results table.
Instead, we design a specialized evaluation protocol in Appendix~B to analyze their stepwise behavior under adapted or constrained settings, allowing us to examine their generation-to-understanding characteristics without penalizing missing architectural capabilities.
...
~\\
\textbf{Takeaway 1: On the majority of tasks, unified models perform worse than their base VL models.}
...
~\\
\textbf{Takeaway 2: Unified models show distinct capability improvements on Spatial Intelligence and Visual Illusions.}
...
\subsection{Comparison Between Stepwise and Direct Inference (RQ2)}

\begin{table}[ht]
\centering
\caption{\textbf{Direct vs.\ Stepwise accuracy on generation-friendly subtasks.}
Stepwise shows consistent improvements in multi-step spatial reasoning regimes.}
...
\begin{figure}[ht]
    ...
    \caption{\textbf{Stepwise vs.\ Direct (Category-level).}
    Each point represents a (model, category) pair comparing Direct and Stepwise accuracy.}
...
\textbf{Takeaway 2.2: Conversely, generating intermediate images provides noticeable performance gains over direct output in transformation-intensive scenarios, where intermediate states can be explicitly grounded and updated.}

Nevertheless, Stepwise generation is not uniformly detrimental. 
A more fine-grained subtask analysis reveals that Stepwise inference yields 
...
Stepwise outperforms Direct for a substantial fraction of models 
...
In such scenarios, Stepwise generation plays a role analogous to visual reasoning. 
...
\textbf{Takeaway 3: The impact of generative assistance is highly predictable: tasks with similar cognitive demands, as well as models sharing identical base weights, consistently exhibit parallel performance trends.}
...
Here, we focus on alignment patterns across tasks and models, and their relationship to stepwise effectiveness.
...
Second, alignment quality strongly correlates with stepwise effectiveness (Section~\ref{sec:rq2}). 
Tasks and models with higher IR/AL scores typically demonstrate larger gains under Stepwise relative to Direct inference, 
...
OmniGen2(stepwise) & 80 & 6 & 81 & 11 & 78 & 23 & 28 & 27 & 39 & 25 & 22 & 9.82 & 49 & 33.5 & 79 \\
UniWorld-V1(stepwise) & 99 & 4 & 99 & 10 & 95 & 19 & 36 & 22 & 47 & 21 & 34 & 6.75 & 14 & 34 & 80 \\
OneCAT-3b(direct) & 85 & 2 & 84 & 8 & 89 & 13 & 30 & 29 & 38 & 20 & 22 & 10.02 & 46 & 33 & 76.68 \\
OneCAT-3b(stepwise) & 87 & 1 & 82 & 7 & 90 & 4 & 23 & 29 & 35 & 21 & 24 & 9.2 & 33 & 34 & 74 \\
JavisGPT(direct) & 86 & 12 & 75 & 13 & 78 & 15 & 36 & 23 & 43 & 22 & 32 & 8.32 & 36 & 28 & 78.6 \\
uni-video(stepwise) & 97 & 18 & 96 & 36 & 79 & 50 & 32 & 21 & 37 & 26 & 32 & 9.82 & 39 & 29 & 61 \\
UniPic2(stepwise) & 94 & 8 & 88 & 24 & 90 & 22 & 32 & 22 & 40 & 23 & 28 & 11.66 & 21 & 14 & 47 \\
STAR-7B(stepwise) & 95 & 7 & 98 & 13 & 93 & 19 & 32 & 21 & 46 & 23 & 29 & 6.75 & 35 & 33 & 79 \\
\midrule
\rowcolor{gray!12}
Qwen2.5-VL-3B* & 89 & 6 & 87 & 9 & 90 & 18 & 30 & 22 & 44 & 22 & 21 & 6.75 & 51 & 31 & 87 \\
UAE(direct) & 89 & 7 & 88 & 8 & 90 & 18 & 31 & 32 & 34 & 26 & 20 & 10 & 45 & 28.5 & 85 \\
UAE(stepwise) & 87 & 6 & 87 & 6 & 88 & 17 & 32 & 22 & 28 & 29 & 23 & 7.98 & 18 & 28 & 71 \\
Ovis-U1(direct) & 91 & 7 & 83 & 10 & 87 & 18 & 28 & 27 & 30 & 21 & 27 & 7.36 & 59 & 32 & 90 \\
Ovis-U1(stepwise) & 75 & 5 & 73 & 15 & 48 & 21 & 30 & 22 & 30 & 27 & 30 & 6.13 & 23 & 19 & 32 \\
\midrule
\rowcolor{gray!12}
yi-6B-vl* & 81 & 17 & 20 & 16 & 84 & 23 & 31 & 18 & 27 & 25 & 33 & 11.66 & 27 & 21 & 19 \\
mio(direct) & 46 & 5 & 36 & 13 & 30 & 27 & 32 & 22 & 35 & 28 & 24 & 14.72 & 35 & 1 & 15 \\
mio(stepwise) & 53 & 15 & 36 & 16 & 28 & 19 & 21 & 23 & 32 & 28 & 27 & 10.43 & 30 & 0 & 13 \\
\midrule
\rowcolor{gray!12}
...
bagel(stepwise) & 81 & 2 & 81 & 17 & 80 & 20 & 30 & 26 & 43 & 40 & 35 & 9.82 & 46 & 19.5 & 91 \\
...
show-o2(stepwise) & 56 & 6 & 51 & 18 & 53 & 31 & 21 & 23 & 32 & 23 & 25 & 9.82 & 35 & 31 & 35 \\
ILLUME+(direct) & 86 & 4 & 84 & 20 & 91 & 26 & 22 & 28 & 30 & 18 & 23 & 6.75 & 36 & 13 & 80 \\
ILLUME+(stepwise) & 78 & 14 & 83 & 16 & 87 & 21 & 18 & 27 & 37 & 17 & 25 & 4.29 & 21 & 20 & 58 \\
\midrule
\rowcolor{orange!8}
...
OmniGen2(stepwise) & 26 & 0 & 29 & 33 & 17 & 18 & 35 & 31 & 23 & 6.02 & 4.6 & 54 & 0.13 & 41 & 42 \\
UniWorld-V1(stepwise) & 38 & 0 & 27 & 22 & 16 & 21 & 38 & 39 & 24 & 6.02 & 14.43 & 47 & 6.14 & 33 & 36 \\
OneCAT-3b(direct) & 34 & 0 & 32 & 30 & 27 & 24 & 13 & 24 & 25 & 6.02 & 8.4 & 50 & 11.18 & 41 & 23 \\
OneCAT-3b(stepwise) & 17 & 0 & 39 & 28 & 23 & 3 & 26 & 23 & 24 & 6.02 & 7.43 & 50 & 3.1 & 37 & 21 \\
JavisGPT(direct) & 42 & 0 & 16 & 24 & 18 & 19 & 31 & 30 & 33 & 8.74 & 0.48 & 51 & 10.44 & 25 & 28 \\
uni-video(stepwise) & 43 & 0 & 23 & 18 & 17 & 10 & 32 & 28 & 26 & 9.64 & 21.9 & 57 & 30.79 & 31 & 27 \\
UniPic2(stepwise) & 33 & 1 & 21 & 23 & 21 & 23 & 35 & 33 & 22 & 4.82 & 11.53 & 48 & 18.71 & 28 & 34 \\
STAR-7B(stepwise) & 29 & 0 & 27 & 22 & 17 & 21 & 39 & 31 & 27 & 6.02 & 6.87 & 46 & 19.72 & 34 & 31 \\
\midrule
\rowcolor{gray!12}
Qwen2.5-VL-3B* & 31 & 0 & 22 & 32 & 24 & 18 & 36 & 31 & 24 & 3.61 & 1.64 & 50 & 0 & 42 & 39 \\
UAE(direct) & 21 & 0 & 26 & 22 & 26 & 20 & 32 & 33 & 19 & 4.82 & 5.17 & 38 & 5.91 & 35 & 26 \\
UAE(stepwise) & 15 & 0 & 26 & 27 & 19 & 19 & 23 & 26 & 18 & 6.02 & 4.6 & 49 & 12.15 & 37 & 28 \\
Ovis-U1(direct) & 47 & 0 & 12 & 28 & 7 & 18 & 23 & 31 & 28 & 8.43 & 13.44 & 49 & 33.39 & 35 & 26 \\
Ovis-U1(stepwise) & 33 & 0 & 27 & 27 & 15 & 11 & 22 & 22 & 19 & 3.61 & 0.25 & 52 & 0.74 & 19 & 15 \\
\midrule
\rowcolor{gray!12}
yi-6B-vl* & 0 & 0 & 24 & 26 & 22 & 11 & 30 & 33 & 22 & 6.02 & 0 & 11 & 0 & 35 & 32 \\
mio(direct) & 0 & 0 & 21 & 24 & 24 & 23 & 25 & 25 & 20 & 3.61 & 0 & 21 & 0 & 30 & 32 \\
mio(stepwise) & 0 & 0 & 32 & 24 & 14 & 22 & 16 & 27 & 17 & 3.61 & 0 & 1 & 0 & 27 & 29 \\
\midrule
\rowcolor{gray!12}
...
bagel(stepwise) & 58 & 2 & 25 & 36 & 27 & 24 & 33 & 41 & 31 & 4.82 & 28.1 & 55 & 19.45 & 38 & 43 \\
...
show-o2(stepwise) & 18 & 0 & 30 & 15 & 0 & 23 & 34 & 30 & 23 & 10.84 & 17.84 & 52 & 18.32 & 43 & 37 \\
ILLUME+(direct) & 15 & 0 & 28 & 29 & 14 & 14 & 31 & 27 & 19 & 6.02 & 6.93 & 53 & 4.02 & 38 & 32 \\
ILLUME+(stepwise) & 4 & 0 & 29 & 28 & 15 & 16 & 17 & 20 & 18 & 2.41 & 12.87 & 52 & 0 & 37 & 34 \\
\midrule
\rowcolor{orange!8}
...
OmniGen2(stepwise) & -18 & 1 & -17 & 1 & -16 & 3 & -3 & 3 & -7 & -1 & -6 & -1.18 & 5 & -3 & -7 \\
UniWorld-V1(stepwise) & 1 & -1 & 1 & 0 & 1 & -1 & 5 & -2 & 1 & -5 & 6 & -4.25 & -30 & -2.5 & -6 \\
OneCAT-3b(direct) & -13 & -3 & -14 & -2 & -5 & -7 & -1 & 5 & -8 & -6 & -6 & -0.98 & 2 & -3.5 & -9.32 \\
OneCAT-3b(stepwise) & -11 & -4 & -16 & -3 & -4 & -16 & -8 & 5 & -11 & -5 & -4 & -1.8 & -11 & -2.5 & -12 \\
JavisGPT(direct) & -12 & 7 & -23 & 3 & -16 & -5 & 5 & -1 & -3 & -4 & 4 & -2.68 & -8 & -8.5 & -7.4 \\
uni-video(stepwise) & -1 & 13 & -2 & 26 & -15 & 30 & 1 & -3 & -9 & 0 & 4 & -1.18 & -5 & -7.5 & -25 \\
UniPic2(stepwise) & -4 & 3 & -10 & 14 & -4 & 2 & 1 & -2 & -6 & -3 & 0 & 0.66 & -23 & -22.5 & -39 \\
STAR-7B(stepwise) & -3 & 2 & 0 & 3 & -1 & -1 & 1 & -3 & 0 & -3 & 1 & -4.25 & -9 & -3.5 & -7 \\
\midrule
\rowcolor{gray!12}
Qwen2.5-VL-3B* & - & - & - & - & - & - & - & - & - & - & - & - & - & - & - \\
UAE(direct) & 0 & 1 & 1 & -1 & 0 & 0 & 1 & 10 & -10 & 4 & -1 & 3.25 & -6 & -2.5 & -2 \\
UAE(stepwise) & -2 & 0 & 0 & -3 & -2 & -1 & 2 & 0 & -16 & 7 & 2 & 1.23 & -33 & -3 & -16 \\
Ovis-U1(direct) & 2 & 1 & -4 & 1 & -3 & 0 & -2 & 5 & -14 & -1 & 6 & 0.61 & 8 & 1 & 3 \\
Ovis-U1(stepwise) & -14 & -1 & -14 & 6 & -42 & 3 & 0 & 0 & -14 & 5 & 9 & -0.62 & -28 & -12 & -55 \\
\midrule
\rowcolor{gray!12}
yi-6B-vl* & - & - & - & - & - & - & - & - & - & - & - & - & - & - & - \\
mio(direct) & -35 & -12 & 16 & -3 & -54 & 4 & 1 & 4 & 8 & 3 & -9 & 3.06 & 8 & -20 & -4 \\
mio(stepwise) & 7 & 10 & 0 & 3 & -2 & -8 & -11 & 1 & -3 & 0 & 3 & -4.29 & -5 & -1 & -2 \\
\midrule
\rowcolor{gray!12}
deepseek-v2* & - & - & - & - & - & - & - & - & - & - & - & - & - & - & - \\
...
bagel(stepwise) & -5 & -4 & 7 & -1 & -11 & -14 & -5 & 1 & 15 & 8 & 13 & -1.22 & -8 & -12.5 & 13 \\
TokenFlow-XL(direct) & 0 & 6 & 1 & -5 & -13 & -19 & 1 & -2 & 15 & -10 & 10 & -2.72 & -18 & -4 & 0.6 \\
show-o2(direct) & 3 & 5 & 6 & 4 & -10 & 3 & -13 & -1 & 8 & -11 & 4 & -3.68 & -16 & 1 & -33 \\
show-o2(stepwise) & -30 & 0 & -23 & 0 & -38 & -3 & -14 & -2 & 4 & -9 & 3 & -1.22 & -19 & -1 & -43 \\
ILLUME+(direct) & 0 & -2 & 10 & 2 & 0 & -8 & -13 & 3 & 2 & -14 & 1 & -4.29 & -18 & -19 & 2 \\
ILLUME+(stepwise) & -8 & 8 & 9 & -2 & -4 & -13 & -17 & 2 & 9 & -15 & 3 & -6.75 & -33 & -12 & -20 \\
\midrule
\rowcolor{orange!8}
gpt4o & - & - & - & - & - & - & - & - & - & - & - & - & - & - & - \\
gpt4o & - & - & - & - & - & - & - & - & - & - & - & - & - & - & - \\
...
OmniGen2(stepwise) & -20 & 0 & 4 & 13 & 7 & -11 & 2 & -5 & -2 & -3.62 & -9.52 & 2 & -5.48 & 7 & 8 \\
UniWorld-V1(stepwise) & -8 & 0 & 2 & 2 & 6 & -8 & 5 & 3 & -1 & -3.62 & 0.31 & -5 & 0.53 & -1 & 2 \\
OneCAT-3b(direct) & -12 & 0 & 7 & 10 & 17 & -5 & -20 & -12 & 0 & -3.62 & -5.72 & -2 & 5.57 & 7 & -11 \\
OneCAT-3b(stepwise) & -29 & 0 & 14 & 8 & 13 & -26 & -7 & -13 & -1 & -3.62 & -6.69 & -2 & -2.51 & 3 & -13 \\
JavisGPT(direct) & -4 & 0 & -9 & 4 & 8 & -10 & -2 & -6 & 8 & -0.9 & -13.64 & -1 & 4.83 & -9 & -6 \\
uni-video(stepwise) & -3 & 0 & -2 & -2 & 7 & -19 & -1 & -8 & 1 & 0 & 7.78 & 5 & 25.18 & -3 & -7 \\
UniPic2(stepwise) & -13 & 1 & -4 & 3 & 11 & -6 & 2 & -3 & -3 & -4.82 & -2.59 & -4 & 13.1 & -6 & 0 \\
STAR-7B(stepwise) & -17 & 0 & 2 & 2 & 7 & -8 & 6 & -5 & 2 & -3.62 & -7.25 & -6 & 14.11 & 0 & -3 \\
\midrule
\rowcolor{gray!12}
Qwen2.5-VL-3B* & - & - & - & - & - & - & - & - & - & - & - & - & - & - & - \\
UAE(direct) & -10 & 0 & 4 & -10 & 2 & 2 & -4 & 2 & -5 & 1.21 & 3.53 & -12 & 5.91 & -7 & -13 \\
UAE(stepwise) & -16 & 0 & 4 & -5 & -5 & 1 & -13 & -5 & -6 & 2.41 & 2.96 & -1 & 12.15 & -5 & -11 \\
Ovis-U1(direct) & 16 & 0 & -10 & -4 & -17 & 0 & -13 & 0 & 4 & 4.82 & 11.8 & -1 & 33.39 & -7 & -13 \\
Ovis-U1(stepwise) & 2 & 0 & 5 & -5 & -9 & -7 & -14 & -9 & -5 & 0 & -1.39 & 2 & 0.74 & -23 & -24 \\
\midrule
\rowcolor{gray!12}
yi-6B-vl* & - & - & - & - & - & - & - & - & - & - & - & - & - & - & - \\
mio(direct) & 0 & 0 & -3 & -2 & 2 & 12 & -5 & -8 & -2 & -2.41 & 0 & 10 & 0 & -5 & 0 \\
mio(stepwise) & 0 & 0 & 11 & 0 & -10 & -1 & -9 & 2 & -3 & 0 & 0 & -20 & 0 & -3 & -3 \\
\midrule
\rowcolor{gray!12}
deepseek-v2* & - & - & - & - & - & - & - & - & - & - & - & - & - & - & - \\
...
bagel(stepwise) & 37 & 2 & 8 & 10 & -2 & 4 & -2 & 10 & 8 & -12.05 & 20.64 & 4 & 10.2 & -4 & -3 \\
TokenFlow-XL(direct) & 21 & 0 & -1 & -2 & -11 & -1 & -4 & -1 & 10 & -8.13 & -6.98 & 0 & 1.19 & -17 & -18 \\
show-o2(direct) & 1 & 0 & 10 & -2 & -8 & 8 & -6 & 1 & -1 & -7.23 & 6.78 & 5 & 4.49 & -5 & -3 \\
show-o2(stepwise) & -3 & 0 & 13 & -11 & -29 & 3 & -1 & -1 & 0 & -6.03 & 10.38 & 1 & 9.07 & 1 & -9 \\
ILLUME+(direct) & -6 & 0 & 11 & 3 & -15 & -6 & -4 & -4 & -4 & -10.85 & -0.53 & 2 & -5.23 & -4 & -14 \\
ILLUME+(stepwise) & -17 & 0 & 12 & 2 & -14 & -4 & -18 & -11 & -5 & -14.46 & 5.41 & 1 & -9.25 & -5 & -12 \\
\midrule
\rowcolor{orange!8}
gpt4o & - & - & - & - & - & - & - & - & - & - & - & - & - & - & - \\
ovis2.5 & - & - & - & - &- & - & - & - & - & - & - & - & - & - & - \\
...
As discussed in Section~5, not all unified models evaluated in this work are trained with explicit image-to-image (edit) capabilities. For such models, stepwise generation that requires intermediate image editing inherently falls outside their training distribution, effectively constituting an out-of-distribution (OOD) setting. Including their Stepwise results directly in the main comparison table could therefore introduce an unfair bias.

To better understand the effect of this OOD intermediate generation, we conduct additional ablation studies on a representative subset of edit-untrained models, with the results summarized in Table~\ref{tab:sheet7_scaled}. Specifically, we categorize their stepwise evaluation into two alternative variants:

\paragraph{(1) OOD-Image Generation (UAE and X-Omni).}
Despite lacking edit training, the first two models are forced to generate an intermediate image under the standard Stepwise protocol (i.e., conditioned on the original input image). The generated image is then re-consumed for answering. This setting measures how severely forcing OOD intermediate generation degrades downstream reasoning performance.
...
Overall, these ablation results validate our decision to exclude the Stepwise scores of edit-untrained models from the main comparison table. They strongly emphasize that Generate-to-Understand (G2U) gains rely critically on explicit and training-aligned intermediate generation capabilities.
...
OmniGen2(stepwise) & 6 & 11 & 29 & 33 & 18 & 19.40~\textcolor{red}{(+$1.60$)} \\
UniWorld-V1(stepwise) & 4 & 10 & 27 & 22 & 21 & 16.80~\textcolor{blue}{(-$1.00$)} \\
OneCAT-3b(direct) & 2 & 8 & 32 & 30 & 24 & 19.20~\textcolor{red}{(+$1.40$)} \\
OneCAT-3b(stepwise) & 1 & 7 & 39 & 28 & 3 & 15.60~\textcolor{blue}{(-$2.20$)} \\
JavisGPT(direct) & 12 & 13 & 16 & 24 & 19 & 16.80~\textcolor{blue}{(-$1.00$)} \\
uni-video(stepwise) & 18 & 36 & 23 & 18 & 10 & \textbf{21.00~\textcolor{red}{(+$3.20$)}} \\
UniPic2(stepwise) & 8 & 24 & 21 & 23 & 23 & 19.80~\textcolor{red}{(+$2.00$)} \\
STAR-7B(stepwise) & 7 & 13 & 27 & 22 & 21 & 18.00~\textcolor{red}{(+$0.20$)} \\
...
UAE(stepwise) & 6 & 6 & 26 & 27 & 19 & 16.80~\textcolor{blue}{(-$0.60$)} \\
Ovis-U1(direct) & 7 & 10 & 12 & 28 & 18 & 15.00~\textcolor{blue}{(-$2.40$)} \\
Ovis-U1(stepwise) & 5 & 15 & 27 & 27 & 11 & 17.00~\textcolor{blue}{(-$0.40$)} \\
...
mio(stepwise) & 15 & 16 & 32 & 24 & 22 & 21.80~\textcolor{red}{(+$3.00$)} \\
